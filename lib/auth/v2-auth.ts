...
async function tryApiKeyAuth(
	...
	try {
		...
		if (!valid || error || !key) {
			return {
				success: false,
				error: (error?.message as string) || "Invalid API key",
			};
		}
		...
		return {
			...
			user: {
				id: key.referenceId,
				email: key.referenceId, // We don't have email from API key, use referenceId as fallback
...
export function getAuthErrorResponse(authResult: AuthenticationResult): {
	...
	return {
		...
		details:
			"Please provide a valid session or API key in the Authorization header",
		status: 401,
	};
}
