...
async function basicExample() {
  try {
    ...
    const receivedEmails = await inbound.mail.list({ limit: 5 })
    console.log('Found', receivedEmails.emails.length, 'received emails')
    
    // List endpoints
    console.log('Listing endpoints...')
    const endpoints = await inbound.endpoints.list({ limit: 5 })
    ...
    const addresses = await inbound.emailAddresses.list({ limit: 5 })
