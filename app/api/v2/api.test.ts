...
describe("Domain Authentication API", () => {
    ...
    describe("GET /domains/{id}/dns-records", () => {
        it("should retrieve DNS records with validation status", async () => {
            if (!domainId) {
                console.error("❌ No domain ID available for DNS records test");
                return;
            }
            ...
            if (response.status !== 200) {
                console.error("❌ DNS records request failed:", data);
                return;
            }
            ...
            // Validate individual records structure
            data.records.forEach((record: any) => {
                ...
                expect(typeof record.isRequired).toBe('boolean');
            ...
            // Test validity logic
            const requiredRecords = data.records.filter((r: any) => r.isRequired);
            const verifiedRequiredRecords = requiredRecords.filter((r: any) => r.isVerified);
            const expectedValid = requiredRecords.length > 0 ? verifiedRequiredRecords.length === requiredRecords.length : true;
            
            expect(data.valid).toBe(expectedValid);

            console.log(`✅ DNS records retrieved - Domain: ${data.domain}, Records: ${data.records.length}, Valid: ${data.valid}, Required: ${requiredRecords.length}, Verified Required: ${verifiedRequiredRecords.length}`);
        });
    });
});