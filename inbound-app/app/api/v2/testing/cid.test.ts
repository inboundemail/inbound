 ...
 * Tests the v2 API's ability to embed images inline using content_id
 * Based on SDK CID test structure
 */
...
describe('CID Image Embedding - v2 API Integration Tests', () => {
  ...
  describe('🎯 Basic CID Functionality', () => {
    ...
    test('should embed logo using content_id with base64 data', async () => {
          ...
          <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <footer style="text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #e9ecef;">
              <p style="color: #999; font-size: 12px;">
                Sent via Inbound Email v2 API with CID support
              </p>
            </footer>
          </body>
          </html>
      ...
      console.log('📊 CID Basic Test Response:', response.status, data)
    ...
    test('should embed logo using content_id with remote URL', async () => {
        ...
        html: `
          ...
          <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="padding: 30px 0;">
              ...
              <p style="color: #666; line-height: 1.6;">
                This test demonstrates CID embedding using a remote file URL. The API fetches 
      ...
      console.log('📊 CID Remote Test Response:', response.status, data)
  ...
  describe('🎨 Advanced CID Scenarios', () => {
    ...
    test('should handle multiple CID images in newsletter layout', async () => {
      ...
      console.log('📊 Multiple CID Test Response:', response.status, data)
    ...
    test('should handle mixed CID and regular attachments', async () => {
      ...
      const emailData = {
        ...
        html: `
          ...
          <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <footer style="text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #e9ecef;">
              <p style="color: #999; font-size: 12px;">
                This email demonstrates mixed CID and regular attachments using Inbound Email v2 API
              </p>
            </footer>
          </body>
          </html>
      ...
      console.log('📊 Mixed Attachments Test Response:', response.status, data)
  ...
  describe('⚠️ Error Handling', () => {
    ...
    test('should handle duplicate content_ids gracefully', async () => {
      ...
      console.log('📊 Duplicate CID Error Response:', response.status, data)

      // Check if API properly validates duplicate content_ids
      if (response.status === 400 && data.error && data.error.includes('Duplicate content_id')) {
        expect(data.error).toContain('Duplicate content_id')
    ...
    test('should handle content_id length validation', async () => {
      ...
      console.log('📊 Long Content ID Error Response:', response.status, data)

      // Check if API properly validates content_id length
      if (response.status === 400 && data.error && data.error.includes('content_id must be less than 128 characters')) {
        expect(data.error).toContain('content_id must be less than 128 characters')
  ...
  describe('📊 Performance & Compatibility', () => {
    ...
    test('should handle large CID images efficiently', async () => {
      ...
      console.log('📊 Large Image Test Response:', response.status, data)
    ...
    test('should validate CID references in HTML', async () => {
      ...
      console.log('📊 CID Reference Test Response:', response.status, data)
...
console.log('🧪 CID Image Embedding Tests Setup Complete')
