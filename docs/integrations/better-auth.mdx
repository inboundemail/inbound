...
For beautiful, responsive emails, use our pre-built React Email templates. First install the peer dependency:

```bash
npm install @react-email/components
```

Then configure specific events with React templates:
...
| `BetterAuthAccountCreated` | Welcome email for new accounts | `userEmail`, `userName`, `timestamp`, `method`, `provider`, `appName`, `supportEmail`, `logoUrl`, `dashboardLink` |
| `BetterAuthTwoFactorEnabled` | 2FA enabled notification | `userEmail`, `userName`, `timestamp`, `method`, `appName`, `supportEmail`, `logoUrl`, `secureAccountLink` |
| `BetterAuthTwoFactorDisabled` | 2FA disabled warning | `userEmail`, `userName`, `timestamp`, `method`, `appName`, `supportEmail`, `logoUrl`, `secureAccountLink` |
