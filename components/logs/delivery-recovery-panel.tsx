...
export function DeliveryRecoveryPanel({
	...
	return (
		<details className="border border-border bg-card p-4 sm:p-6 [font-family:'Helvetica_Neue',sans-serif]">
			<summary className="cursor-pointer text-sm font-medium">
				<span id={headingId}>Delivery details</span>
				{needsAttention && (
			...
			<section aria-labelledby={headingId} className="mt-4">
				...
				{disabledReason && (
					<p
						role="status"
						className="mt-4 text-sm text-destructive break-words"
					>
						{disabledReason}
					</p>
				)}
				{deliveries.length === 0 ? (
					<div className="mt-4 space-y-1 text-sm">
						...
						{!disabledReason && (
							<p>
								Choose an active endpoint with “Send to endpoint” to deliver
								this email.
							</p>
						)}
					</div>
								...
								>
									...
									<dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm sm:grid-cols-4">
										...
										<div>
											...
											<dd className="mt-1 tabular-nums">
												{lastAttempt &&
												Number.isFinite(lastAttempt.getTime()) ? (
